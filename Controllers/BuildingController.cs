...
namespace VibeCity_API.Data
{
    ...
    public class Student
    {
        ...
        // 🌟 THÊM DÒNG NÀY VÀO ĐỂ MAP VỚI SUPABASE
    ...
    public class BuildingController : ControllerBase
    {
        ...
        public async Task<IActionResult> CompleteNight([FromBody] QuitGameProgressRequest request)
        {
            try
            {
                ...
                // 1. Cộng dồn số ngày sống sót (Đã có sẵn)
                student.SurvivedDays += request.SurvivedDays;

                // 🌟 THÊM DÒNG NÀY ĐỂ CỘNG DỒN SỐ PHÚT CHƠI THỰC TẾ XUỐNG SUPABASE
                student.TotalSurvivalMinutes += actualMinutes;

                // 2. Lưu thay đổi (Đã có sẵn)
        ...
        // --- Sửa file BuildingController.cs ---

        // API ẩn dùng để đánh thức Server Render khi người chơi vừa mở game
        [AllowAnonymous]
        [HttpGet("ping")]
        public IActionResult PingServer()
        {
            // In ra màn hình console của Render để ông dễ theo dõi xem Unity đã "gõ cửa" chưa
    ...
    } // ✅ ĐÃ THÊM: Dấu ngoặc đóng Class BuildingController chuẩn xác!

    // --- CÁC CLASS DTO SẠCH SẼ NẰM NGOÀI CONTROLLER ---
...
namespace VibeCity_API.Data
{
    ...
    public class BuildingController : ControllerBase
    {
        ...
        public async Task<IActionResult> Post([FromBody] BuildingDto data)
        {
            ...
            // 🔒 Lấy ID từ Token trực tiếp, chống giả mạo thông tin người xây
        ...
        public async Task<ActionResult<IEnumerable<BuildingDto>>> GetBuildings([FromQuery] string studentId, [FromQuery] bool isChung)
        {
            try
            {
                ...
                {
                    // 🔒 Nếu tải map cá nhân, ép buộc hệ thống dùng ID từ Token để bảo mật dữ liệu riêng tư
                    buildings = await _context.Buildings
        ...
        public async Task<IActionResult> Login([FromBody] Student loginInfo)
        {
            ...
            var tokenDescriptor = new SecurityTokenDescriptor
            {
                Subject = new ClaimsIdentity(new[]
                {
                    ...
                    new Claim("FullName", student.FullName),
                    new Claim("Major", student.Major),
                    new Claim("Gpa", student.Gpa.ToString()) // Thêm để Unity check cổng mở Map 2 nhanh
                }),
                Expires = DateTime.UtcNow.AddDays(7),
                Issuer = _configuration["Jwt:Issuer"],
                Audience = _configuration["Jwt:Audience"],
