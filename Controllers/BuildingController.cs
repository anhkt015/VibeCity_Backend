...
namespace VibeCity_API.Data
{
    ...
    public class BuildingController : ControllerBase
    {
        ...
        // =======================================================
        // API NHẬN THƯỞNG CỔNG BÌNH MINH (BỔ SUNG THÊM)
        // =======================================================
        [Authorize]
        ...
        public async Task<IActionResult> ClaimDawnReward([FromBody] ClaimDawnGateRequest body, CancellationToken cancellationToken)
        {
            ...
            if (string.IsNullOrWhiteSpace(studentId))
            {
                return Unauthorized(new { success = false, message = "Không xác định được danh tính người chơi." });
            }

            await using var transaction = await _context.Database.BeginTransactionAsync(cancellationToken);

            // Kiểm tra xem đã nhận thưởng cho đêm này chưa
            bool alreadyClaimed = await _context.DawnGateRewards
                .AnyAsync(x => x.StudentId == studentId && x.NightCycleId == body.NightCycleId, cancellationToken);

            if (alreadyClaimed)
            {
                return BadRequest(new { success = false, message = "Bạn đã nhận phần thưởng cho đêm này rồi!" });
            }
            ...
            {
                StudentId = studentId,
                NightCycleId = body.NightCycleId,
                RewardAmount = 50,
